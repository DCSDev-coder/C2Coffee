...
export default function StaffGuides() {
  ...
  return <div className="p-6 lg:p-10 space-y-7">
    ...
    <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{guides.map((guide) => <article key={guide.id} className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm"><img src={`${getAdminApiBaseUrl()}${guide.image_url}`} alt={guide.guide_title || guideLabels[guide.guide_type]} className="h-56 w-full object-contain bg-gray-50" /><div className="flex items-center gap-3 p-4"><BookOpen size={18} className="text-[#2E5E58]"/><div className="min-w-0 flex-1"><p className="font-bold">{guide.menu_item_name || guide.guide_title || guideLabels[guide.guide_type]}</p><p className="truncate text-sm text-gray-500">{guide.guide_type === 'drink' ? 'Drink preparation' : guideLabels[guide.guide_type]}</p></div><button onClick={() => removeGuide(guide.id)} className="rounded-lg p-2 text-red-600 hover:bg-red-50" aria-label="Delete guide"><Trash2 size={18}/></button></div></article>)}</div>
