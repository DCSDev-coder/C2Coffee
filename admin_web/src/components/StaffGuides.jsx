...
export default function StaffGuides() {
  ...
  const [type, setType] = useState('attire'); const [menuItemId, setMenuItemId] = useState('');
  ...
  const addGuide = async (event) => {
    ...
    try {
      ...
      await adminRequest('/v1/admin/barista-guides', { method: 'POST', body: JSON.stringify({ guide_type: type, menu_item_id: type === 'drink' ? Number(menuItemId) : null, image_url: upload.image_url }) });
      setFile(null); setMenuItemId(''); await load();
  ...
  return <div className="p-6 lg:p-10 space-y-7">
    ...
    <form onSubmit={addGuide} className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm grid gap-4 md:grid-cols-4 items-end">
      <label className="text-sm font-semibold text-gray-700">Guide type<select value={type} onChange={(event) => setType(event.target.value)} className="mt-2 w-full rounded-lg border border-gray-300 p-3"><option value="attire">Attire guide</option><option value="rules">Store rules</option><option value="drink">Drink preparation</option></select></label>
    ...
    <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{guides.map((guide) => <article key={guide.id} className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm"><img src={`${getAdminApiBaseUrl()}${guide.image_url}`} alt={guideLabels[guide.guide_type]} className="h-56 w-full object-contain bg-gray-50" /><div className="flex items-center gap-3 p-4"><BookOpen size={18} className="text-[#2E5E58]"/><div className="min-w-0 flex-1"><p className="font-bold">{guideLabels[guide.guide_type]}</p><p className="truncate text-sm text-gray-500">{guide.menu_item_name || 'All baristas'}</p></div><button onClick={() => removeGuide(guide.id)} className="rounded-lg p-2 text-red-600 hover:bg-red-50" aria-label="Delete guide"><Trash2 size={18}/></button></div></article>)}</div>
    {!guides.length && <div className="rounded-2xl border border-dashed border-gray-300 p-12 text-center text-gray-500"><ImagePlus className="mx-auto mb-3"/>No staff guide images have been uploaded.</div>}
  </div>;
}
