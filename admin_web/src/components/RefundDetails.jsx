...
const StatusBadge = ({ status }) => (
  <span
    className={`inline-flex rounded-md px-2.5 py-1 text-xs font-bold ${statusClass[status] || "bg-slate-100 text-slate-700"}`}
  >
    {status}
  </span>
);
...
const formatAmount = (refund) => {
  const isTokenRefund =
    String(refund.paymentMethod || "").toLowerCase() === "token";
  if (isTokenRefund) {
...
const RefundDetails = ({ onBack, currentUser }) => {
  ...
  const reviewRefund = async (refund, decision, password) => {
    ...
    try {
      ...
      setError(
        requestError.message || "Unable to record this refund decision.",
      );
    } finally {
      setActionId("");
    }
  ...
  const filtered = refunds.filter((refund) => {
    ...
    const matchesSearch =
      !query ||
      [refund.id, refund.orderId, refund.customer, refund.reason].some(
        (value) =>
          String(value || "")
            .toLowerCase()
            .includes(query),
    ...
    return (
      matchesSearch &&
      (statusFilter === "All Status" || refund.status === statusFilter)
    );
  });
  ...
  const counts = Object.fromEntries(
    STATUSES.slice(1).map((status) => [
      status,
      refunds.filter((refund) => refund.status === status).length,
    ]),
  );
  ...
  return (
    <div className="flex h-full flex-col px-8 pb-8 pt-2">
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <h1 className="text-2xl font-bold text-gray-900">Refund Reviews</h1>
          </div>
          <p className="ml-8 mt-0.5 text-sm text-gray-500">
            Approve a request to return its tokens to the customer's wallet.
          </p>
        </div>
        <button
          onClick={() => setIsCreateOpen(true)}
          className="rounded-lg bg-[#1F3A34] px-4 py-2 text-sm font-bold text-white hover:bg-[#2E5E58]"
        >
      ...
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative w-full sm:max-w-md">
          ...
          <input
            ...
            placeholder="Search refund, order, customer or reason"
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-6 xl:flex-row">
              ...
              <thead>
                <tr className="text-left text-xs font-bold text-gray-900">
                  {[
                    ...
                    <th key={heading} className="px-5 py-4 whitespace-nowrap">
                      {heading}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-gray-100">
                {loading ? (
                  ...
                  pageItems.map((refund) => (
                    <tr
                      key={refund.id}
                      className={`cursor-pointer transition-colors ${selectedRefund?.id === refund.id ? "bg-[#F3F7F5]" : "hover:bg-gray-50"}`}
        ...
        {selectedRefund && (
          <aside className="w-full shrink-0 rounded-xl border border-gray-200 bg-white p-5 shadow-sm xl:w-[360px]">
            ...
            <div className="mt-5 space-y-4 border-y border-gray-100 py-4 text-sm">
              <div>
                ...
                <p className="text-gray-600">{selectedRefund.email}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500">Order and amount</p>
                <p className="font-semibold text-gray-900">
                  {selectedRefund.orderId} · {formatAmount(selectedRefund)}
                </p>
              </div>
              <div>
            ...
            <button
              onClick={() =>
                setViewingProfileFor({
                  username: selectedRefund.customer,
                  email: selectedRefund.email,
                  phone: selectedRefund.phone,
                })
              }
              className="mt-4 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm font-bold text-gray-700 hover:bg-gray-50"
            >
              View Customer
            </button>
            {selectedRefund.email && (
              <a
                href={`mailto:${selectedRefund.email}?subject=${encodeURIComponent(`Refund request ${selectedRefund.id}`)}`}
                className="mt-2 flex w-full items-center justify-center gap-2 rounded-lg border border-gray-300 px-3 py-2 text-sm font-bold text-gray-700 hover:bg-gray-50"
              >
                <Mail size={15} /> Email Customer
              </a>
            )}
            {selectedRefund.status === "Pending" && canReviewRefunds && (
              <div className="mt-4 grid grid-cols-2 gap-2">
                <button
                  disabled={actionId === selectedRefund.id}
                  onClick={() =>
                  ...
                  className="rounded-lg border border-red-300 px-3 py-2 text-sm font-bold text-red-700 hover:bg-red-50 disabled:opacity-50"
                >
                  Reject
                </button>
                <button
                  ...
                  className="flex items-center justify-center gap-2 rounded-lg bg-[#1F3A34] px-3 py-2 text-sm font-bold text-white hover:bg-[#2E5E58] disabled:opacity-50"
                >
                  <Check size={16} />
                  {actionId === selectedRefund.id ? "Saving..." : "Approve"}
                </button>
              </div>
            )}
            {selectedRefund.status === "Pending" && !canReviewRefunds && (
              <p className="mt-4 text-sm text-gray-500">
                A Super Admin must review this request.
              </p>
            )}
          </aside>
        )}
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-500">
                  Use this when support receives a request outside the app.
                </p>
              </div>
              <button
                type="button"
          ...
          >
            <h2 className="text-lg font-bold text-gray-900">
              {pendingConfirmation.type === "create"
                ? "Create refund request?"
                : `${pendingConfirmation.decision === "approved" ? "Approve" : "Reject"} refund?`}
            </h2>
            <p className="mt-2 text-sm text-gray-600">
              Confirm this action with your current administrator password. This
              decision is recorded in the refund history.
            </p>
            <label className="mt-5 block text-sm font-bold text-gray-700">
              Current password
              <input
